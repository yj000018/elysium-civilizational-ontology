...
def run_audit():
    ...
    audit_prompt = f"""
Perform Chief Architect Audit #2 on Documents 06-10 (Ontology and Knowledge block).

The audit must review:
* architectural coherence (does the Knowledge Graph spec match the Ontology spec?)
* document dependency consistency
* alignment with ELYSIUM purpose (epistemic infrastructure)
* governance implications (who curates the Model Registry?)
...
"""
    print("Running Chief Architect Audit #2...")
    audit_report = call_llm(audit_prompt, architect_system)
    
    with open("/home/ubuntu/ELYSIUM/00_PROGRAM_OFFICE/AUDIT_02.md", "w") as f:
    ...
    state["audits"][1]["status"] = "COMPLETED"
    state["next_action"] = "Start Document 11: Governance Handbook"
    state["active_document"] = "11"
    
    with open("/home/ubuntu/ELYSIUM/00_PROGRAM_OFFICE/ELYSIUM_PROGRAM_STATE.json", "w") as f:
        json.dump(state, f, indent=2)
        
    print("Audit #2 complete and state updated.")
